...
3，容器的生命周期
    ...
    -- 暂停： docker pause 容器id或name 
    
    -- 关闭:  docker stop 容器id或name
    
    -- 重启： docker restart 容器id或name
    
    -- 查看日志信息： docker logs 容器id或name
       有用选项: --tail n 查看倒数n行
                 -f 实时查看docker终端的日志输出
       两者可以结合使用 docker logs --tail 10 -f 容器id或者name
...
5,容器运行时 动作：
    ...
    -- 容器中执行新命令，作用：在容器中运行一个命令： docker exec -it 容器id或者name  命令[比如/bin/bash] 
